...
export function AnalysisProvider({ children }) {
  ...
  const fetchLatestData = useCallback(async (id) => {
    ...
    try {
      ...
      const [analysisRes, carbonRes] = await Promise.all([
        axios.get(`${BASE_URL}/api/v1/analysis/latest/${id}`, noCache),
        axios.get(`${BASE_URL}/api/analysis/carbon/stats`, {
  ...
  const fetchEcoPreview = useCallback(async (id) => {
    if (!id) return;
    try {
      const res = await axios.get(`${BASE_URL}/api/v1/analysis/eco/preview/${id}`);
  ...
  const fetchBenchmarkData = useCallback(async (id) => {
    ...
    try {
      const res = await axios.get(
        `${BASE_URL}/api/analysis/benchmark/company/${id}`,
  ...
  const value = {
    companyId, setCompanyId: setCompanyIdPersisted,
    companyProfileName,
